...
bool frame_tracker::motion_based_track(data::frame& curr_frm, const data::frame& last_frm, const Mat44_t& velocity) const {
    ...
    if (curr_frm.odom_updated_) {
        ...
        spdlog::info("\nodom mat\n{}\ntwist mat\n{}\n,twist_only\n{}\nold frame{}", toString(odom_position), toString(velocity * last_frm.cam_pose_cw_), toString(velocity), toString(last_frm.cam_pose_cw_));
    }
    else {
        //this uses frame n-1 and frame n-2 to determine where frame n is expected to be
